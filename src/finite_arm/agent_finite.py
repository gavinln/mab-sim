...
class FiniteBernoulliBanditBootstrap(FiniteBernoulliBanditTS):
    ...
    def get_posterior_sample(self):
        ...
        boot_sample = (
            np.random.binomial(total_tries, prob_success) / total_tries
        )
        return boot_sample
...
class FiniteBernoulliBanditLaplace(FiniteBernoulliBanditTS):
    ...
    def get_posterior_sample(self):
        ...
        laplace_sample = mode + np.sqrt(1 / hessian) * np.random.randn(
            self.n_arm
        )
...
class DriftingFiniteBernoulliBanditTS(FiniteBernoulliBanditTS):
    ...
    def update_observation(self, observation, action, reward):
        ...
        self.prior_success = (
            self.prior_success * (1 - self.gamma) + self.a0 * self.gamma
        )
        self.prior_failure = (
            self.prior_failure * (1 - self.gamma) + self.b0 * self.gamma
        )
...
class FiniteBernoulliBanditLangevin(FiniteBernoulliBanditTS):
    ...
    def compute_gradient(self, x):
        grad = (self.prior_success - 1) / x - (self.prior_failure - 1) / (
            1 - x
        )
        return grad

    def compute_preconditioners(self, x):
        second_derivatives = (self.prior_success - 1) / (x**2) + (
            self.prior_failure - 1
        ) / ((1 - x) ** 2)
    ...
    def get_posterior_sample(self):
        ...
        preconditioner, preconditioner_sqrt = self.compute_preconditioners(
            x_map
        )
