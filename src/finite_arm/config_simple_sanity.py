...
def get_config():
    ...
    agents = collections.OrderedDict(
        [
            (
                'Laplace TS',
                functools.partial(FiniteBernoulliBanditLaplace, n_arm),
            ),
            (
                'Langevin TS',
                functools.partial(
                    FiniteBernoulliBanditLangevin, n_arm, step_count, step_size
                ),
            ),
            (
                'bootstrap TS',
    ...
    for env in range(n_env):
        ...
        environments[env] = functools.partial(
            FiniteArmedBernoulliBandit, probs
        )
