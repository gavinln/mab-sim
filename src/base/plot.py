...
def cumulative_travel_time_plot(experiment_name, data_path=_DEFAULT_DATA_PATH):
    ...
    plt_df = (
        df.groupby(['t', 'agent']).agg({'cum_ratio': np.mean}).reset_index()
    )
    p = (
        gg.ggplot(plt_df)
...
def plot_action_proportion(df_agent):
    ...
    p = (
        gg.ggplot(pd.melt(plt_df, id_vars='t'))
...
def compare_action_selection_plot(
    experiment_name='finite_simple', data_path=_DEFAULT_DATA_PATH
):
...
def misspecified_plot(
    experiment_name='finite_misspecified', data_path=_DEFAULT_DATA_PATH
):
    ...
    def _parse_np_array(np_string):
        return np.array(
            np_string.replace('[', '').replace(']', '').strip().split()
        )
...
def ensemble_plot(experiment_name='ensemble_nn', data_path=_DEFAULT_DATA_PATH):
    ...
    for agent_family, df_family in plt_df.groupby(['agent_family']):
        if agent_family == 'Ensemble':
            ...
            gg_legend = gg.scale_colour_manual(
                values=custom_colors, labels=custom_labels, name='agent'
            )
        else:
...
def concurrent_agents_plot(
    experiment_name='graph_indep_concurrent',
    ...
    plt_df_per_action = (
        df.groupby(['agent', 't', 'agent_id', 'action_id'])
        .agg({'instant_regret': np.mean})
        .reset_index()
    )
...
def rename_agent(name):
    if name == 'Laplace TS':
        ...
        raise ValueError(
            'There is an unknown agent. Perhaps you need to use other plotting functions.'
        )


def customized_algorithm_plot(
    experiment_name='finite_simple_sanity', data_path=_DEFAULT_DATA_PATH
):
    """Simple plot of average instantaneous regret by agent, per timestep.
